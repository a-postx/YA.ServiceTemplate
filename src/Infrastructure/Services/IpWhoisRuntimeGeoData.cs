...
public class IpWhoisRuntimeGeoData : IRuntimeGeoDataService
{
    ...
    private async Task<IpWhoisGeoData> GetDataAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            using (Stream responseStream = await response.Content.ReadAsStreamAsync(cancellationToken))
            {
                IpWhoisGeoData data = await JsonSerializer
                    .DeserializeAsync<IpWhoisGeoData>(responseStream, new JsonSerializerOptions(), cancellationToken);
        ...
        {
            _log.LogWarning("Unable to get runtime geodata: {ExceptionMessage}", ex.Message);
        }
        catch (Exception ex)
        {
